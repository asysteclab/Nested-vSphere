...
$VIServer = "vc-01.lab.adms.local"
$VIUsername = "ccooney"

$VMPassword = read-host -prompt "ESXi Password"
$VCSASSOPassword = read-host -prompt "VCSA SSO Password"
$VCSARootPassword = read-host -prompt "VCSA Root Password"
...
$NestedESXiApplianceOVA = "..\Nested_ESXi6.0u3_Appliance_Template_v1.0.ova"
$VCSAInstallerPath = "..\VMware-VCSA-all-6.0.0-5326177"
...
$VCSADisplayName = "nlvc-01"
$VCSAIPAddress = "10.27.0.97"
$VCSAHostname = "nlvc-01.ccc.lab.adms.local" #Change to IP if you don't have valid DNS
$VCSAPrefix = "24"
$VCSASSODomainName = "vsphere.local"
$VCSASSOSiteName = "NestedLab"

$VCSASSHEnable = "true"

# General Deployment Configuration for both Nested ESXi VMs + VCSA
$VMNetwork = "vxw-dvs-55-virtualwire-42-sid-5013-NST-GuestVM-CCC-LS"
$VMDatastore = "ATLL-VSAN-DS"
$VMNetmask = "255.255.255.0"
$VMGateway = "10.27.0.1"
$VMDNS = "172.16.5.100"
$VMNTP = "pool.ntp.org"

$VMDomain = "ccc.lab.adms.local"
$VMSyslog = "172.16.5.127"
...
$verboseLogFile = "lab-deployment.log"
$vSphereVersion = "6.0u3"
...
if($deployVCSA -eq 1) {
    ...
    if($DeploymentTarget -eq "ESXI") {
        ...
        $config.'target.vcsa'.vc.target = 'ATLL-CLU'
        $config.'target.vcsa'.appliance.'deployment.network' = $VMNetwork
        $config.'target.vcsa'.appliance.'thin.disk.mode' = $true
        $config.'target.vcsa'.appliance.'deployment.option' = $VCSADeploymentSize
        $config.'target.vcsa'.appliance.name = $VCSADisplayName
        ...
        My-Logger "Creating VCSA JSON Configuration file for deployment ..."
        $config | ConvertTo-Json |  % { [System.Text.RegularExpressions.Regex]::Unescape($_) } | Set-Content -Path "$($ENV:Temp)\jsontemplate.json"

        My-Logger "Deploying the VCSA ..."
        $output = Invoke-Expression "$($VCSAInstallerPath)\vcsa-cli-installer\win32\vcsa-deploy.exe install --no-esx-ssl-verify --accept-eula $($ENV:Temp)\jsontemplate_working.json" -ErrorVariable vcsaDeployOutput 2>&1
        $vcsaDeployOutput | Out-File -Append -LiteralPath $verboseLogFile
    }
}
...
if($moveVMsIntovApp -eq 1 -and $DeploymentTarget -eq "VCENTER") {
	...
	Move-VApp -VApp $VAppName -Destination "CCC"
}
...
if($setupNewVC -eq 1) {
    ...
    if($addESXiHostsToVC -eq 1) {
        $NestedESXiHostnameToIPs.GetEnumerator() | sort -Property Value | Foreach-Object {
            ...
            Add-VMHost -Server $vc -Location VSAN-Cluster -User "root" -Password $VMPassword -Name $VMIPAddress -Force | Out-File -Append -LiteralPath $verboseLogFile
        }
